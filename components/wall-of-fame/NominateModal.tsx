...
const CATEGORIES = [
  { value: 'moment',  label: '🏆 Legendary Moment' },
  { value: 'quote',   label: '💬 Epic Quote' },
  { value: 'trip',    label: '✈️ Trip Story' },
  { value: 'hostel',  label: '🏫 Hostel Era' },
  { value: 'cricket', label: '🏏 Cricket Tale' },
  { value: 'music',   label: '🎵 Music Memory' },
]
...
export function NominateModal({ onClose, onCreated }: NominateModalProps) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="The time Chenga ate 7 plates of biriyani..."
              required
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-secondary border border-border rounded-xl px-4 py-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/30 transition"
            >
              {CATEGORIES.map(c => (
          ...
          <div>
            <label className="text-sm font-medium text-foreground block mb-1.5">
              Era <span className="text-muted-foreground font-normal">(optional)</span>
            </label>
            <select
              value={era}
          ...
          {/* Story */}
          <div>
            <label className="text-sm font-medium text-foreground block mb-1.5">The Story *</label>
            <textarea
              value={story}
              onChange={(e) => setStory(e.target.value)}
              placeholder="Tell us what happened. Every detail counts. The Banglans deserve to know..."
              required
              rows={5}
            ...
            />
            <p className="text-xs text-muted-foreground/60 mt-1">{story.length} / 2000 characters</p>
          </div>
